...
from kamo.grasp import grasp_from_upper_left_side
from kamo.mapping import get_place_from_label, get_pose_from_place
from kamo.task1 import goto_properly
from nakagawa.lib3d_detection import get_most_graspable_obj, go_back_and_watch
from nakagawa.open_drawers import open_drawers
from nakagawa.our_calc_score_box import our_calc_score_box
from nakagawa.specialized_mapping import fix_mistakes
...
class WrsMainController(object):
    ...
    HAND_PALM_OFFSET = 0.05
        # hand_palm_linkは指の付け根なので、把持のために少しずらす必要がある
    ...
    def __init__(self):
        ...
        self.tf_from_bbox_clt = \
            rospy.ServiceProxy(tf_from_bbox_srv_name, SetTransformFromBBox)

        obj_detection_name = "detection/get_object_detection"
        rospy.wait_for_service(obj_detection_name)
        self.detection_clt = \
            rospy.ServiceProxy(obj_detection_name, GetObjectDetection)
    ...
    def get_relative_coordinate(self, parent, child):
        ...
        try:
            ...
            log_str = (
                "failed to get transform between "
                f"[{parent}] and [{child}]\n")
    ...
    def extract_target_obj_and_person(instruction):
        ...
        # pattern = r'(?P<object>.*?) to person (?P<person>.*?)'
        # result = re.match(pattern, instruction)
        # print(result, result.group("object"), result.group("person"))
        # target_obj = result.group("object")
        # target_person = result.group("person")
    ...
    def exec_graspable_method(self, grasp_pos, label=""):
        ...
        elif label in ["tuna_fish_can",]:
            method = lambda grasp_pos: grasp_from_upper_left_side(self, grasp_pos)
        else:
    ...
    def deliver_to_target(self, target_obj, target_person):
        ...
        self.change_pose("look_at_near_floor")
        self.goto_name("person_b" if target_person == "right" else "person_a")
    ...
    def execute_avoid_blocks(self):
        ...
        target_pos1 = [2.7, 2.4, -45]
        rospy.loginfo("Starting to move towards target position (%.2f, %.2f, %.2f)",
                      target_pos1[0], target_pos1[1], target_pos1[2])
        self.goto_name("target_pos1")

        target_pos2 = [0, 0, 0]
        rospy.loginfo("Starting to move towards target position (%.2f, %.2f, %.2f)",
                      target_pos2[0], target_pos2[1], target_pos2[2])
        self.goto_name("target_pos2")
        ...
        for i in range(3):
            ...
            # 選択されたウェイポイントに移動
            rospy.loginfo("Moving to waypoint: (%.2f, %.2f, %.2f)", 
                          waypoint[0],
                          waypoint[1],
                          waypoint[2])
    ...
    def execute_task1(self):
        ...
        rospy.loginfo("step 1. open drawers")
        open_drawers(self)

        current_dest = [0.0, 0.0, 0.0]
        ...
        for plc, pose in hsr_position:
            for _ in range(self.DETECT_CNT):
                # 移動と視線指示
                goto_properly(self.coordinates, self.poses, plc, current_dest)
                self.change_pose(pose)
                # gripper.command(0)

                # 引きで把持対象を一度把握
                back_detected = go_back_and_watch(self, pose)
                self.goto_name(plc)
                ...
                graspable_obj = get_most_graspable_obj(
                    detected_objs.bboxes, back_detected,
                    self.IGNORE_LIST)
                ...
                # labelに応じたplaceを計算する
                # 編集: 中川
                fixedlabel = fix_mistakes(label)
                place = get_place_from_label(fixedlabel)
                rospy.loginfo(
                    "put " + label + 
                    " into " + place + " by "+ get_pose_from_place(place))
                pose = "put_in_bin"

                # binに入れる
                goto_properly(self.coordinates, self.poses, place, current_dest)
                self.put_in_place(place, pose)
                total_cnt += 1
